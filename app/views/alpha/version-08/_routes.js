...
router.get("/claims/create/cancel-confirmation", (req, res)=> {
    res.render("alpha/version-07/claims/create/cancel-confirmation", {referer: req.headers.referer});
});
...
router.post('/route-cancel-create-claim', function(req,res){
    ...
    if (cancel == "yes" ) {
        res.redirect('/alpha/version-07/claims/index')
    } else if (cancel == "no"){
        return res.redirect(back)
    } else {
        res.redirect('/alpha/version-07/claims/create/cancel-confirmation')
    }
...
router.post('/route-upload-method', function(req,res){
    ...
    if (method == "RINA" ) {
        res.redirect('/alpha/version-07/claims/invoices/upload/xml/index')
        }
    else if (method == "EDI"){
            res.redirect('/alpha/version-07/claims/invoices/upload/xml/index')
            }
    else {
        res.redirect('/alpha/version-07/claims/invoices/upload/index')
    }
...
router.get("/claims/invoices/upload/cancel-confirmation", (req, res)=> {
    res.render("alpha/version-07/claims/invoices/upload/cancel-confirmation", {referer: req.headers.referer});
});
...
router.post('/route-cancel-upload-invoices', function(req,res){
    ...
    if (cancel == "yes" ) {
        res.redirect('/alpha/version-07/claims/summary/not-started')
    } else if (cancel == "no"){
        return res.redirect(back)
    } else {
        res.redirect('/alpha/version-07/claims/invoices/upload/cancel-confirmation')
    }
...
router.get("/claims/invoices/upload/delete-confirmation", (req, res)=> {
    res.render("alpha/version-07/claims/invoices/upload/delete-confirmation", {referer: req.headers.referer});
});
...
router.post('/route-delete-invoice-file', function(req,res){
    ...
    if (cancel == "yes" ) {
        ...
        res.redirect('/alpha/version-07/claims/invoices/upload/delete-confirmation')
    }
...
router.get("/claims/invoices/upload/xml/additional-delete-confirmation", (req, res)=> {
    res.render("alpha/version-07/claims/invoices/upload/xml/additional-delete-confirmation", {referer: req.headers.referer});
});
...
router.get('/claims/create/cya', function(req, res) {
    ...
    res.render('alpha/version-07/claims/create/cya', {fullSubmissionMonth: fullSubmissionMonth, fullTargetMonth: fullTargetMonth});
})
...
router.post('/create-claim-validation', function(req,res){
    ...
    if (
        ...
        {
        res.redirect('/alpha/version-07/claims/create/error')
        }
    else {
        res.redirect('/alpha/version-07/claims/create/cya')
    }
...
router.post('/route_delete_claim', function(req,res){
    ...
    if (confirmation == "yes" ) {
        res.redirect('/alpha/version-07/post-mvp/claim-summary/delete-confirmation')
        }
    else if (confirmation == "no"){
            res.redirect('/alpha/version-07/post-mvp/claim-summary/not-started')
            }
    else {
        res.redirect('/alpha/version-07/post-mvp/claim-summary/delete-check')
    }
...
router.post('/route-additional-upload', function(req,res){
    ...
    if (upload == "yes" ) {
        res.redirect('/alpha/version-07/claims/invoices/upload/xml/index-additional-file')
        }
    else if (upload == "no"){
            res.redirect('/alpha/version-07/claims/invoices/upload/xml/cya')
            }
    else {
        res.redirect('/alpha/version-07/claims/invoices/upload/xml/additional-file')
    }
...
router.post('/route-additional-upload-2', function(req,res){
    ...
    if (upload == "yes" ) {
        res.redirect('/alpha/version-07/claims/invoices/upload/xml/additional-bulk-upload')
            ...
            res.redirect('/alpha/version-07/claims/invoices/upload/xml/additional-bulk-cya')
            }
    else {
        res.redirect('/alpha/version-07/claims/invoices/upload/xml/additional-file-2')
    }
...
router.get('/claims/summary/not-started', function(req, res) {
    ...
    res.render('alpha/version-07/claims/summary/not-started', { targetDateEditFormatted: targetDateEditFormatted, targetDateFormatted: targetDateFormatted });
})

// DYNAMIC SEARCH and RESULTS RENDERING
...
const pool = new Pool({
    ...
    ssl: process.env.NODE_ENV === 'production' ? {
        rejectUnauthorized: false  // Only for Heroku or if your DB requires SSL
    } : false
});
...
router.post('/personSearch', async (req, res) => {
    const {
      ...
      uniqueID, issuePin, residenceCountry
    } = req.body;
    ...
    try {
        const query = `
            SELECT * FROM claims
            WHERE 
            ($1::text IS NULL OR data->'person'->>'lastName' ILIKE $1) AND
            ($2::text IS NULL OR data->'person'->>'firstName' ILIKE $2) AND
            ($3::text IS NULL OR (data->'person'->'dob'->>'day')::int = $3::int) AND
            ($4::text IS NULL OR (data->'person'->'dob'->>'month')::int = $4::int) AND
            ($5::text IS NULL OR (data->'person'->'dob'->>'year')::int = $5::int) AND
            ($6::text IS NULL OR data->'person'->'address'->>'residenceCountry' ILIKE $6) AND
        ...
        const params = [
            ...
            uniqueID ? JSON.stringify({ "ohs": uniqueID }): null,
            uniqueID ? JSON.stringify({ "nino": uniqueID }): null,
            issuePin ? JSON.stringify({ "issueNumber": issuePin }) : null, // Checks for issueNumber
            issuePin ? JSON.stringify({ "pin": issuePin }) : null // Checks for pin
        ];
        const result = await pool.query(query, params);
        res.render('s1-default-search-results', { searchResults: result.rows });
    } catch (err) {
...
//       res.render('version-07/claims/invoices/details/person-not-found/s1-default-search-results', { searchResults: filteredResults });
//     });
//   });
