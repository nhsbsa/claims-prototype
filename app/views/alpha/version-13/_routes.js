...
router.post('/route_create_new_submission_6784', function(req,res){
    var submissionType = req.session.data['type']
    if (submissionType == "Contestation") {
        res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/submissions/create/02a-contestation-actual')
        }
        else if (submissionType == "Credit note") {
            res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/submissions/create/02b-credit-note-actual')
            }
    else {
        res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/submissions/create/01-submission-type')
    }
})
...
router.post('/route-20001288-mark-complete', function(req,res){
    ...
    if (ageBracket == "0 to 19 years" || ageBracket == "20 to 64 years" || ageBracket == "65 years and over") {
        res.redirect('/alpha//version-13/claims/rina/average-cost/spain/5422/invoices/20001288/02-confirmation')
        }
    else {
        ...
        if (accepted == "0") {
            ...
            res.redirect('/alpha//version-13/claims/rina/average-cost/spain/5422/invoices/20001288/01b-to-check-error')
        }
    }
...
router.post('/route-claim-search', function(req,res){
    ...
    if (claimId == "6784" || claimId == "7399") {
        res.redirect('/alpha/version-13/claims/search/individual-claim')
        }
    else {
        res.redirect('/alpha/version-13/claims/search/no-results')
    }
})
...
router.post('/route-contact-method', function(req,res){
    ...
    if (received == "RINA" ||  received == "Paper") {
        res.redirect('/alpha/version-13/claims/create/type')
        }
    else {
        res.redirect('/alpha/version-13/claims/create/index')
    }
})
...
router.post('/route-create-claim', function(req,res){
    ...
    if (received == 'RINA') {
        ...
        if (claimType == "Actual cost - Sickness" || claimType == "Actual cost - Accident at work") {
            res.redirect('/alpha/version-13/claims/create/details-rina-actual')
            }

        else if (claimType == "Average cost - Sickness") {
            if (articleType == "Article 63 (2a)" || articleType == "Article 63 (2b)") {
                res.redirect('/alpha/version-13/claims/create/details-rina-average-63')
                }
            else if (articleType == "Article 94" || articleType == "Article 95") {
                res.redirect('/alpha/version-13/claims/create/details-rina-average-94-95')
                }
            else {
                    res.redirect('/alpha/version-13/claims/create/type')
                }
        ...
        if (claimType == "Actual cost - Sickness" || claimType == "Actual cost - Accident at work") {
            res.redirect('/alpha/version-13/claims/create/details-paper-actual')
            }

        else if (claimType == "Average cost - Sickness") {
            if (articleType == "Article 63 (2a)" || articleType == "Article 63 (2b)") {
                res.redirect('/alpha/version-13/claims/create/details-paper-average-63')
                }
            else if (articleType == "Article 94" || articleType == "Article 95") {
            res.redirect('/alpha/version-13/claims/create/details-paper-average-94-95')
            }
            else {
                res.redirect('/alpha/version-13/claims/create/type')
            }
    ...
    if (received == 'RINA') {
        if (article == "Article 62 - Sickness" || article == "Article 62 - Accident at work") {
            res.redirect('/alpha/version-13/claims/create/details-rina-actual')
                ...
                res.redirect('/alpha/version-13/claims/create/article-type')
            }
            }
...
router.get("/claims/create/cancel-confirmation", (req, res)=> {
    res.render("alpha/version-13/claims/create/cancel-confirmation", {referer: req.headers.referer});
});
...
router.post('/route-cancel-create-claim', function(req,res){
    ...
    if (cancel == "yes" ) {
        res.redirect('/alpha/version-13/claims/index')
    } else if (cancel == "no"){
        return res.redirect(back)
    } else {
        res.redirect('/alpha/version-13/claims/create/cancel-confirmation')
    }
...
router.post('/route-upload-method', function(req,res){
    ...
    if (method == "RINA" ) {
        res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/invoices/upload/xml/index')
        }
    else {
        res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/invoices/upload/index')
    }
...
router.get("/claims/invoices/upload/cancel-confirmation", (req, res)=> {
    res.render("alpha/version-13/claims/invoices/upload/cancel-confirmation", {referer: req.headers.referer});
});
...
router.post('/route-cancel-upload-invoices', function(req,res){
    ...
    if (cancel == "yes" ) {
        res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/summary/not-started')
    } else if (cancel == "no"){
        return res.redirect(back)
    } else {
        res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/invoices/upload/cancel-confirmation')
    }
...
router.get("/claims/invoices/upload/delete-confirmation", (req, res)=> {
    res.render("alpha/version-13/claims/invoices/upload/delete-confirmation", {referer: req.headers.referer});
});
...
router.post('/route-delete-invoice-file', function(req,res){
    ...
    if (cancel == "yes" ) {
        res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/invoices/upload/xml/index?show-back=No')
    } else if (cancel == "no"){
        res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/invoices/upload/xml/cya')
    } else {
        res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/invoices/upload/xml/delete-confirmation')
    }
...
router.get("/claims/invoices/upload/xml/additional-delete-confirmation", (req, res)=> {
    res.render("alpha/version-13/claims/invoices/upload/xml/additional-delete-confirmation", {referer: req.headers.referer});
});
...
router.post('/route-delete-invoice-additional-file', function(req,res){
    ...
    if (cancel == "yes" ) {
        res.redirect('/alpha/version-13/claims/invoices/upload/xml/additional-bulk-upload')
    } else if (cancel == "no"){
        return res.redirect(back)
    } else {
        res.redirect('/alpha/version-13/claims/invoices/upload/delete-confirmation')
    }
...
router.get('/claims/create/cya', function(req, res) {
    ...
    res.render('alpha/version-13/claims/create/cya', {fullSubmissionMonth: fullSubmissionMonth, fullTargetMonth: fullTargetMonth});
})
...
router.post('/create-claim-validation', function(req,res){
    ...
    if (
        ...
        {
        res.redirect('/alpha/version-13/claims/create/error')
        }
    else {
        res.redirect('/alpha/version-13/claims/create/cya')
    }
...
router.post('/route_delete_claim', function(req,res){
    ...
    if (confirmation == "yes" ) {
        res.redirect('/alpha/version-13/post-mvp/claim-summary/delete-confirmation')
        }
    else if (confirmation == "no"){
            res.redirect('/alpha/version-13/post-mvp/claim-summary/not-started')
            }
    else {
        res.redirect('/alpha/version-13/post-mvp/claim-summary/delete-check')
    }
...
router.post('/route-additional-upload', function(req,res){
    ...
    if (upload == "yes" ) {
        res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/invoices/upload/xml/index-additional-file')
        }
    else if (upload == "no"){
            res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/invoices/upload/xml/cya')
            }
    else {
        res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/invoices/upload/xml/additional-file')
    }
...
router.post('/route-additional-invoice', function(req,res){
    ...
    if (type == "bulk" ) {
        res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/invoices/upload/xml/additional-bulk-upload')
        }
    else if (type == "single"){
            res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/invoices/create/index')
            }
    else {
        res.redirect('/alpha/version-13/claims/rina/actual-cost/6784-france/invoices/upload/options')
    }
...
router.post('/route-additional-upload-2', function(req,res){
    ...
    if (upload == "yes" ) {
        res.redirect('/alpha/version-13/claims/invoices/upload/xml/additional-bulk-upload')
            ...
            res.redirect('/alpha/version-13/claims/invoices/upload/xml/additional-bulk-cya')
            }
    else {
        res.redirect('/alpha/version-13/claims/invoices/upload/xml/additional-file-2')
    }
...
router.get('/claims/summary/not-started', function(req, res) {
    ...
    res.render('alpha/version-13/claims/summary/not-started', { targetDateEditFormatted: targetDateEditFormatted, targetDateFormatted: targetDateFormatted });
})
...
router.post('/personSearch', (req, res) => {
    ...
    fs.readFile(jsonDataPath, (err, data) => {
      ...
      res.render('version-13/claims/invoices/details/person-not-found/s1-default-search-results', { searchResults: filteredResults });
    });
  });
