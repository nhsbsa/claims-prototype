...
router.post('/claimSearch', async (req, res) => {
    ...
    try {
        ...
        res.render('alpha/version-10/claims/search-results', {
...
router.post('/claimShortSearch', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(query, params);
        res.render('alpha/version-10/claims/short-search-results', {
            searchResults: result.rows,
            searchQuery: req.body,
        });
    } catch (err) {
...
router.post('/invoiceShortSearch', async (req, res) => {
    ...
    try {
        ...
        res.render('alpha/version-10/claims/rina/invoices/invoice-list-search-results', {
            searchResults: result.rows
        });
    } catch (err) {
        console.error('Error executing search query:', err);
...
router.get('/claims/claim/:claimid', async (req, res) => {
    ...
    try {
        ...
        res.render('alpha/version-10/claims/summary/claim-summary', { claim });
...
router.get('/claims/invoices/:claimid', async (req, res) => {
    ...
    try {
        ...
        res.render('alpha/version-10/claims/rina/invoices/full-invoice-list', {
            claimid,
...
router.post('/invoiceSearch', async (req, res) => {
    ...
    try {
        ...
        res.render('alpha/version-10/claims/rina/invoices/full-invoice-list-search-results', {
            searchResults: result.rows,
            claimid,
            country,
            totalInvoices
...
router.get('/claims/invoice/:invoiceid', async (req, res) => {
    ...
    try {
        ...
        res.render('alpha/version-10/claims/rina/invoices/details/_01-to-check', {
            invoice,
            claim,
            contestationReasons
        });
...
// function processSearchInputs(req) {
//     const {
//         lastName, firstName, day, month, year,
//         residenceCountry, ohs, nino, nhs, pin, issueNumber,
//         'sort-by': sortByField, 'order-by': orderByDirection,
//         'filter-entitlement': entitlementFilters
//     } = req.body;

//     // Validate and process National Insurance number
//     let ninoProcessed = null;
//     if (nino && validateNino(nino)) {
//         ninoProcessed = nino.substring(0, 8); // Ignore last character
//     }

//     // Determine if the search is using a wildcard
//     const processedLastName = lastName
//         ? lastName.includes('%')
//             ? lastName.toLowerCase() // Wildcard search, case-insensitive
//             : lastName // Normal search, case-sensitive
//         : null;

//     const processedFirstName = firstName
//         ? firstName.includes('%')
//             ? firstName.toLowerCase() // Wildcard search, case-insensitive
//             : firstName // Normal search, case-sensitive
//         : null;

//     // Partial search for DOB
//     const processedDay = day ? parseInt(day, 10) : null;
//     const processedMonth = month ? parseInt(month, 10) : null;
//     const processedYear = year ? parseInt(year, 10) : null;

//     // Ensure entitlements are an array
//     const entitlementsArray = Array.isArray(entitlementFilters) ? entitlementFilters : [entitlementFilters].filter(Boolean);
...
//         year: processedYear,
//         residenceCountry: residenceCountry ? `%${residenceCountry.toLowerCase()}%` : null,
//         ohs: ohs || null,
//         nino: ninoProcessed,
//         nhs: nhs || null,
//         pin: pin ? `${pin}%` : null,
//         issueNumber: issueNumber ? `${issueNumber}%` : null,
//         sortBy: sortByField || 'lastname',  // Default to 'lastname'
//         orderBy: orderByDirection || 'desc',   // Default to 'desc'
...
router.post('/personSearch', async (req, res) => {
    ...
    try {
        // Mapping sortBy value to database columns
        const sortByMapping = {
            'lastname': 'lastname',
            'firstname': 'firstname',
        ...
        const sortColumn = sortByMapping[sortBy] || 'lastname';
        const sortDirection = orderBy === 'desc' ? 'DESC' : 'ASC';

        const lastNameCondition = lastName && lastName.endsWith('%') ? 'ILIKE' : '=';
        const firstNameCondition = firstName && firstName.endsWith('%') ? 'ILIKE' : '=';
        ...
        const personQuery = `
            ...
            FROM persons
            WHERE 
            ($1::text IS NULL OR lastname ${lastNameCondition} $1) AND
            ($2::text IS NULL OR firstname ${firstNameCondition} $2) AND
            ($3::int IS NULL OR dob_day = $3::int AND dob_month = $4::int AND dob_year = $5::int) AND
        ...
        res.render('alpha/version-10/claims/rina/invoices/details/_02-person-search-results', {
...
// Route to render person search results with filtering
router.post('/personFilterSearch', async (req, res) => {
    try {
        const {
            lastName, firstName, day, month, year,
            residenceCountry, ohs, nino, nhs, pin, issueNumber,
            page = 1, limit = 10, sortBy = 'lastname', orderBy = 'asc',
            'filter-entitlement': filterEntitlement
        } = req.body;
        ...
        const entitlementArray = Array.isArray(filterEntitlement) ? filterEntitlement : filterEntitlement ? [filterEntitlement] : [];

        // Correct mapping of sortBy value to database columns
        const sortByMapping = {
            'lastname': 'lastname',
            'firstname': 'firstname',
            'dob': 'dob_year, dob_month, dob_day',  // Adjusted mapping for DOB sorting
            'reference': "uniqueid->>'reference'",
            'ohs': "uniqueid->>'ohs'"
        };
        ...
        // Build the entitlement condition with OR logic for multiple selections
        let entitlementCondition = '';
        if (entitlementArray.length > 0) {
            entitlementCondition = entitlementArray.map(e => `entitlement_element->>'type' ILIKE '%${e}%'`).join(' OR ');
            entitlementCondition = `AND (${entitlementCondition})`;
        }
        ...
        const personQuery = `
            SELECT 
                ...
                entitlements,
                CASE
                    WHEN entitlement_element->>'type' IN (${entitlementArray.map(e => `'${e}'`).join(', ')}) THEN
                        regexp_replace(entitlement_element->>'type', '.*(S1|S2|DA1|GHIC|EHIC|PRC).*', '\\1')
                    ELSE
                        'Unknown'
                END AS entitlement
            FROM persons, jsonb_array_elements(entitlements::jsonb) AS entitlement_element
            WHERE 
            ($1::text IS NULL OR lastname ${lastNameCondition} $1) AND
            ($2::text IS NULL OR firstname ${firstNameCondition} $2) AND
            ($3::int IS NULL OR dob_day = $3::int AND dob_month = $4::int AND dob_year = $5::int) AND
            ...
            ORDER BY ${sortColumn} ${sortDirection}  -- Correctly order by the selected column
            LIMIT $12 OFFSET $13;
        `;
        ...
        console.log('Query result:', JSON.stringify(personResult.rows, null, 2));

        res.render('alpha/version-10/claims/rina/invoices/details/_02-person-search-results', {
            searchResults: personResult.rows,
            invoice,
            totalPersons: personResult.rows.length,
            page: parseInt(page, 10),
            limit: parseInt(limit, 10),
            totalPages: Math.ceil(personResult.rows.length / limit),
            queryParams: {
                lastName, firstName, day, month, year,
                residenceCountry, ohs, nino, nhs, pin, issueNumber,
                filterEntitlement, sortBy, orderBy
            }
        });
    } catch (err) {
...
router.get('/03-accepted', async (req, res) => {
    ...
    try {
        ...
        res.render('alpha/version-10/claims/rina/invoices/details/_03-accepted', { 
            invoice,
            person: {
                lastname: person.lastname,
                firstname: person.firstname,
